class Solution(object):
    def isMatch(self, s, p):
        ...
        #    print i
        # for dp_list in matching_dp_list:
        #     print dp_list
        return matching_dp_list[len(s)][len(p)]

    def match_question(self, cnt_s, cnt_p, x, y, matching_dp_list):
        if len(cnt_s) == 0:
            return False
    ...
    def match_asterisk(self, cnt_s, cnt_p, x, y, matching_dp_list, s, p):
        if cnt_s == "":
            return cnt_p == "*" and (matching_dp_list[x][y - 1])
        return matching_dp_list[x - 1][y - 1] or matching_dp_list[x - 1][y] or matching_dp_list[x][y - 1]
        '''
    ...
    def match(self, s, p, x, y, matching_dp_list):
        ...
        if cnt_p == "?":
            # print x, y, "match_dot"
            return self.match_question(cnt_s, cnt_p, x, y, matching_dp_list)
        elif cnt_p == "*":
